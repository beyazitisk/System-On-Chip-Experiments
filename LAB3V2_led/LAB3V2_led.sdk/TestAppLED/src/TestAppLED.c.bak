...
int main (void)
{
    ...
    status = XGpio_Initialize(&leds, XPAR_SWITCHES_DEVICE_ID); // XPAR_SWITCHES_DEVICE_ID parameter has been defined in xparameters.h
	if (status != XST_SUCCESS)
		return XST_FAILURE;

	XGpio_SetDataDirection(&leds, 1, 0x00000000); // Setting the direction of the GPIO unit as input by assigning it to 0xffffffff
	...
	while (1) // infinite loop
	{
	  ...
	  XGpio_DiscreteWrite(&leds,1,&dip_switches);
	  sleep(1);
	}
